...
void CommandLine::sendWebUIStatus() {
  int scan_mode = wifi_scan_obj.currentScanMode;
  int ap_count = access_points->size();
  int sta_count = stations->size();
  int ble_count = 0;
  #ifdef HAS_BT
    ble_count = ble_devices->size();
  #endif
  unsigned long uptime_sec = millis() / 1000;

  Serial.print(F("@WEBUI:{"));
  Serial.print(F("\"mode\":"));
  Serial.print(scan_mode);
  Serial.print(F(",\"aps\":"));
  Serial.print(ap_count);
  Serial.print(F(",\"stas\":"));
  Serial.print(sta_count);
  Serial.print(F(",\"ble\":"));
  Serial.print(ble_count);
  Serial.print(F(",\"ver\":\""));
  Serial.print(version_number);
  Serial.print(F("\",\"board\":\""));
  Serial.print(HARDWARE_NAME);
...
void CommandLine::RunSetup() {
  ...
  Serial.println(F("\n\n--------------------------------\n"));
  Serial.println(F("         ESP32 dmatrix      \n"));
